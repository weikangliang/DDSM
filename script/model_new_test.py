...
class Model(object):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False):
        ...
        with tf.name_scope('Embedding_layer'):
            self.mask_embedding = tf.get_variable("mask_embedding", [1, 1, 1, EMBEDDING_DIM * 2])
            # ---------------------------------------uid_embedding---------------------------------------
        ...
        if self.use_negsampling:
            self.noclk_item_his_eb = tf.concat([self.noclk_mid_his_batch_embedded[:, :, 0, :], self.noclk_cat_his_batch_embedded[:, :, 0, :]], -1)  # [128,20,5,36] 0 means only using the first negative item ID. 3 item IDs are inputed in the line 24.
        ...
        size = 20
        # 生成一个20x20的单位矩阵
        identity_matrix = tf.eye(size)
        # 生成一个全为1的矩阵
        ones_matrix = tf.ones([size, size])
        # 生成所需矩阵：单位矩阵取反后与全1矩阵相加
        # 生成所需矩阵：单位矩阵取反后与全1矩阵相加
        result_matrix = ones_matrix - identity_matrix
    ...
    def bert_aux_loss(self, inputs, noclick_seq, emb_dim=36, seq_len=20, stag="bert_aux_net", use_dice=False):  # [128,20,36], [128,20,5,36]
        pos_items_emb = tf.reshape(inputs, [-1, emb_dim])  # [128,20,36]->[2560,36]
        pos_items_emb = tf.expand_dims(pos_items_emb, axis=1)  # [2560,36]->[2560,1,36]
        neg_items_emb = tf.reshape(noclick_seq, [-1, tf.shape(noclick_seq)[2], emb_dim])  # [128,20,5,36]-> [2560,5,36]

        identity_matrix = tf.eye(seq_len)  # [20,20]生成一个20x20的单位矩阵
        ones_matrix = tf.ones([seq_len, seq_len])  # [20,20]生成一个全为1的矩阵
        mask_matrix_origin = ones_matrix - identity_matrix  # [20,20]得到对角线为0的矩阵

        inputs_expand = tf.tile(tf.expand_dims(inputs, 1), [1, seq_len, 1, 1])  # [128,20,36]->[128,1,20,36]->[128,20,20,36]

        mask_matrix = tf.expand_dims(tf.expand_dims(mask_matrix_origin, -1), 0)  # [20,20]->[1,20,20,1]
        mask_matrix = tf.tile(mask_matrix, [tf.shape(inputs)[0], 1, 1, tf.shape(inputs)[-1]])  # [1,20,20,1]->[128,20,20,36]

        mask_embedding_expand = tf.tile(self.mask_embedding, [tf.shape(inputs)[0], seq_len, seq_len, 1])  # [1,1,1,36]->[128,20,20,36]
        mask_embedding_expand_with_pos = tf.concat([mask_embedding_expand, tf.tile(tf.expand_dims(self.position_his_items_eb, 1), [1, seq_len, 1, 1])], axis=-1)
        mask_embedding_expand_with_pos = tf.layers.dense(mask_embedding_expand_with_pos, emb_dim)

        # 使用 mask_matrix 将对应位置替换成mask_embedding
        inputs_expand_replaced = tf.where(tf.equal(mask_matrix, 0), mask_embedding_expand_with_pos, inputs_expand)  # [128,20,20,36]
        inputs_expand_replaced_reshape = tf.reshape(inputs_expand_replaced, [-1, seq_len, emb_dim])  # [128,20,20,36]->[2560,20,36]

        mask_expand = tf.tile(tf.expand_dims(self.mask, 1), [1, seq_len, 1])  # [128,1,20]->[128,20,20]
        mask_expand_reshape = tf.reshape(mask_expand, [-1, seq_len])  # [128,20,20]->[2560,20]

        multihead_attention_outputs = self_multi_head_attn(inputs_expand_replaced_reshape, num_units=emb_dim, num_heads=1, padding_mask=mask_expand_reshape, causality_mask_bool=False, dropout_rate=0, is_training=self.is_training, name="111")
        multihead_attention_outputs1 = tf.layers.dense(multihead_attention_outputs, emb_dim * 2, activation=tf.nn.relu)
        multihead_attention_outputs1 = tf.layers.dense(multihead_attention_outputs1, emb_dim)  # [2560,20,36]
        multihead_attention_outputs1 = multihead_attention_outputs1 + multihead_attention_outputs
        multihead_attention_outputs2 = self_multi_head_attn(multihead_attention_outputs1, num_units=emb_dim, num_heads=1, padding_mask=mask_expand_reshape, causality_mask_bool=False, dropout_rate=0, is_training=self.is_training, name="222")
        multihead_attention_outputs3 = tf.layers.dense(multihead_attention_outputs2, emb_dim * 2, activation=tf.nn.relu)
        multihead_attention_outputs3 = tf.layers.dense(multihead_attention_outputs3, emb_dim)  # [2560,20,36]
        multihead_attention_outputs3 = multihead_attention_outputs3 + multihead_attention_outputs2
        sequence_output = tf.reshape(multihead_attention_outputs3, tf.shape(inputs_expand))  # [2560,20,36]-> [128,20,20,36]

        padding_mask_zero = tf.zeros_like(sequence_output)  # [128,20,20,36]
        sequence_output = tf.where(tf.equal(mask_matrix, 1), padding_mask_zero, sequence_output)  # [128,20,20,36]
        sequence_output = tf.reshape(tf.reduce_sum(sequence_output, axis=2), [-1, 36])  # [128,20,20,36]->[128,20,36]->[2560,36]

        click_prop_ = self.masked_item_prediction(sequence_output, pos_items_emb)[:, :, 0]  # [2560,36],[2560,1,36]->[2560,1,2]->[2560,1]
        noclick_prop_ = self.masked_item_prediction(sequence_output, neg_items_emb)[:, :, 1]  # [2560,36],[2560,5,36]->[2560,5,2]->[2560,5]
        mask_reshape = tf.reshape(self.mask, [-1, 1])  # [128,20]->[2560,1]
        click_loss_ = - tf.log(click_prop_) * mask_reshape  # [2560,1]
        noclick_loss_ = - tf.reduce_sum(tf.log(noclick_prop_), axis=-1, keepdims=True) * mask_reshape  # [2560,5]->[2560,1]
        # 设置一个阈值，决定保留损失的比例
        keep_prob = 0.5  # 例如，保留50%的损失
        # 创建一个随机遮罩
        random_mask = tf.random.uniform(tf.shape(click_loss_)) < keep_prob
        # 将随机遮罩转换为浮点数
        random_mask = tf.cast(random_mask, tf.float32)
        # 将遮罩应用到损失上
        masked_click_loss_ = click_loss_ * random_mask
        masked_noclick_loss_ = noclick_loss_ * random_mask
        # 创建一个布尔遮罩，标记非零元素
        count_nonzero = tf.reduce_sum(tf.cast(tf.not_equal(masked_click_loss_, 0.0),tf.float32))
        loss_ = tf.reduce_sum(masked_click_loss_ + masked_noclick_loss_) / tf.reduce_sum(count_nonzero)  # [128,19]->scalar
        # mip_loss = self.infoNCE_loss(pos_logits, neg_logits, self.mask, temperature=1)  # [2560,1],[2560,5],[128,20]
        return loss_, tf.reshape(sequence_output, tf.shape(inputs))  # [128,20,36]

    def masked_item_prediction(self, sequence_output, target_item, hidden_size=36, stag="masked_item_prediction"):  # [2560,36],[2560,1,36],[2560,5,36]
        sequence_output = tf.tile(tf.expand_dims(sequence_output, 1), [1, tf.shape(target_item)[1], 1])  # [2560,36]->[2560,1,36]or[2560,36]->[2560,5,36]
        input = tf.concat([sequence_output, target_item], axis=-1)  # [2560,1,36],[2560,1,36]->[2560,1,72]
        ...
        y_hat = tf.nn.softmax(dnn3) + 0.00000001  # [2560,1,2]or[2560,5,2]
        return y_hat

        # sequence_output = tf.layers.dense(sequence_output, hidden_size, activation=tf.nn.relu)  # [2560,1,36]->[2560,1,36]
        # sequence_output = tf.layers.dense(sequence_output, hidden_size, activation=tf.nn.relu)  # [2560,1,36]->[2560,1,36]
        # sequence_output = tf.layers.dense(sequence_output, hidden_size)  # [2560,1,36]->[2560,1,36]
        # score = tf.reduce_mean(tf.multiply(sequence_output, target_item), axis=-1)  # [2560,1,36],[2560,1,36]->[2560,1]or[2560,5,36],[2560,1,36]->[2560,5]
        # return score

    def infoNCE_loss(self, pos_logits, neg_logits, mask, temperature=1):  # [2560,1],[2560,5],mask:[128,20]
        # 合并正负样本得分
        logits = tf.concat([pos_logits, neg_logits], axis=1)  # [2560,1],[2560,5]->[2560,6]
        # 创建标签，正样本位置为1，负样本位置为0
        labels = tf.concat([tf.ones_like(pos_logits), tf.zeros_like(neg_logits)], axis=1)  # [2560,6]
        # 计算交叉熵损失
        loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits / temperature, labels=labels)  # [2560]
        mask_expand = tf.reshape(mask, [-1])  # [128,20]->[2560]
        return tf.reduce_mean(loss * mask_expand)
    ...
    def auxiliary_loss(self, h_states, click_seq, noclick_seq, mask, stag=None, focal_loss_bool=False):
        ...
        if focal_loss_bool:
            ...
            noclick_loss_ = self.focal_loss(1.0 - noclick_prop_) * mask  # [128,19]
        else:
...
class myModel1(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        # 截断目标物品的购买用户的长度和时间长度：mapping_to_k处理一个给定的序列（或批量序列），使得每个序列中只保留最后 k 个元素，并生成一个对应的掩码。item_bh_seq_len_t返回的是截断后的真实长度
        # 此处的目的是为了方便修改利用的用户序列长度，而不用每次都在prepare_data中修改，prepare_data中保留较长的序列，这里可以随意控制序列的长短
        self.item_bh_k_user_emb, self.item_bh_seq_len_t, self.item_bh_mask_t = mapping_to_k(self.item_user_his_uid_batch_embedded, self.seq_len_u_ph, k=SEQ_USER_T)  # 得到[128,50,18],[128],[128,50,1]
        self.item_bh_time_emb_padding, _, _ = mapping_to_k(self.item_bh_time_embeeded, self.seq_len_u_ph, k=SEQ_USER_T)  # 得到[128,50,18]

        self.item_user_his_items_emb_truncated, _, _ = mapping_to_k(sequence=tf.reshape(self.item_user_his_items_emb, [tf.shape(self.item_user_his_items_emb)[0], tf.shape(self.item_user_his_items_emb)[1], -1]), seq_len=self.seq_len_u_ph, k=SEQ_USER_T)  # [128,50,20,36]->[128,50,20*36],[128,50]->[128*50]得到[128,30,20*36],[128],[128,30,1]
        self.item_user_his_items_emb = tf.reshape(self.item_user_his_items_emb_truncated, [tf.shape(self.item_user_his_items_emb)[0], SEQ_USER_T, -1, INC_DIM])  # [128,30,20*36]->[128,30,20,36]
...
class myModel09112_test(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        with tf.name_scope('target_item_his_users_representation'):
            ...
            # context_input = tf.concat([self.uid_emb, self.target_item_emb,self.item_his_users_axis1_sum, self.item_his_users_axis2_sum, tf.reshape(self.item_bh_k_user_emb, [-1, SEQ_USER_T * EMBEDDING_DIM])], axis=-1)  # [128,50].[128,18],[128,50*18]->[128,776]
...
class myModel09155_test(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        self.item_bh_k_user_emb, self.item_bh_seq_len_t, self.item_bh_mask_t = mapping_to_k(self.item_user_his_uid_batch_embedded, self.seq_len_u_ph, k=SEQ_USER_T)  # [128,50,18],[128],SEQ_USER_T-> [128,50,18],[128],[128,50,1]
        self.item_bh_time_emb_padding, _, _ = mapping_to_k(self.item_bh_time_embeeded, self.seq_len_u_ph, k=SEQ_USER_T)  # [128,50,18],[128],SEQ_USER_T-> [128,50,18],[128],[128,50,1]
        # sequential modeling for item behaviors
        with tf.name_scope('target_item_his_users_representation'):
            # -----------------------------------------------------目标物品的购买用户过Transformer_model的表示 -----------------------------------------------------
            # 对历史物品的用户和时间做element-wise_sum_pooling
...
class myModel09172_test(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        with tf.name_scope('target_item_his_users_representation'):
            ...
            context_input = tf.concat([self.uid_emb, self.target_item_emb, self.item_his_users_axis1_sum, self.item_his_users_axis2_sum, tf.reshape(item_bh_drink_trm_input, [-1, SEQ_USER_T * 38])], axis=-1)  # [128,18],[128,36],[128,38],[128,50],[128,50*38]->[128,2042]
            context_weights_matrix = get_context_matrix(context_input, [weight_matrix_size], is_training=self.is_training)
    ...
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        with tf.name_scope('DMIN_user_module'):
            ...
            with tf.name_scope("multi_head_attention_for_items"):
                ...
                multihead_attention_outputss = self_multi_head_attn_v2(his_item_with_tiv_emb, num_units=EMBEDDING_DIM * 2, num_heads=4, padding_mask=self.mask, causality_mask_bool=True, dropout_rate=0., is_training=self.is_training, name="multihead_attention_outputss")
                for i, multihead_attention_outputs_v2 in enumerate(multihead_attention_outputss):
                    # 下面两行是point-wise feed_forward，每次for循环都会创建新的dense层，这里每个头不共享参数
                    multihead_attention_outputs3 = tf.layers.dense(multihead_attention_outputs_v2, EMBEDDING_DIM * 4, activation=tf.nn.relu)
                    multihead_attention_outputs3 = tf.layers.dense(multihead_attention_outputs3, EMBEDDING_DIM * 2)
...
class myModel_base_test_aux(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        super(myModel_base_test_aux, self).__init__(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling)
        # ----------------------------------------------------- user module -----------------------------------------------------
        his_items_maxlen = 20
        position_embedding_size = 2
        ...
        self.context_embedding = tf.concat([self.position_his_items_eb, self.his_items_tiv_emb], -1)  # [128,20,2],[128,20,18]->[128,20,20]
        with tf.name_scope("multi_head_attention_1"):
            self.noclk_his_eb_with_pos = tf.concat([tf.tile(tf.expand_dims(self.position_his_items_eb, 2), [1, 1, tf.shape(self.noclk_his_eb)[2], 1]),  # [128,20,2]->[128,20,1,2]
                                                    tf.tile(tf.expand_dims(self.his_items_tiv_emb, 2), [1, 1, tf.shape(self.noclk_his_eb)[2], 1]),  # [128,20,18]
                                                    self.noclk_his_eb], axis=-1)  # [128,20,5,36]
            self.noclk_his_eb_with_pos = tf.layers.dense(self.noclk_his_eb_with_pos, EMBEDDING_DIM * 2)

            his_item_with_tiv_emb = tf.concat([self.position_his_items_eb, self.his_items_tiv_emb, self.his_item_emb], axis=-1)  # [128,20,2],[128,20,18],[128,20,36]->[128,20,56]
            his_item_with_tiv_emb = tf.layers.dense(his_item_with_tiv_emb, EMBEDDING_DIM * 2)
            ...
            context_weights_matrix = get_context_matrix(context_input, embed_dims=[weight_matrix_size], dropout_rate=0., is_training=self.is_training)
        self.noclk_his_eb_with_pos = tf.expand_dims(self.noclk_his_eb_with_pos[:, :, 0, :], 2)  # [128,20,1,36]
        aux_loss_1, sequence_outputs = self.bert_aux_loss(his_item_with_tiv_emb, self.noclk_his_eb_with_pos)  # [128,20,36],[128,20,5,36]
        self.aux_loss = aux_loss_1

        # sequence_outputs_att_output, sequence_outputs_attention_score, sequence_outputs_attention_scores_no_softmax = din_attention_with_context(self.target_item_emb, sequence_outputs, self.context_embedding, self.mask, stag="Attention_layer_for_items_sequence_outputs")
        # sequence_outputs_att_fea = tf.reduce_sum(sequence_outputs_att_output, 1)  # [128,1,36]->[128,36]
        ...
        inp = tf.concat([self.uid_emb, self.target_item_emb, self.item_his_eb_sum, self.target_item_emb * self.item_his_eb_sum], 1)
        with tf.name_scope("multi_head_attention_for_items"):
            # multihead_attention_outputss = context_aware_multi_head_self_attention_v2(sequence_outputs + his_item_with_tiv_emb, context_weights_matrix, num_heads=4, num_units=9, padding_mask=self.mask, causality_mask_bool=False, dropout_rate=0.2, is_training=self.is_training, name="multihead_attention_outputss")
            multihead_attention_outputss = self_multi_head_attn_v2(sequence_outputs + his_item_with_tiv_emb, num_units=EMBEDDING_DIM * 2, num_heads=4, padding_mask=self.mask, causality_mask_bool=False, dropout_rate=0., is_training=self.is_training, name="multihead_attention_outputss")
            for i, multihead_attention_outputs_v2 in enumerate(multihead_attention_outputss):
                # 下面两行是point-wise feed_forward，每次for循环都会创建新的dense层，这里每个头不共享参数
                multihead_attention_outputs3 = tf.layers.dense(multihead_attention_outputs_v2, EMBEDDING_DIM * 4, activation=tf.nn.relu)
                multihead_attention_outputs3 = tf.layers.dense(multihead_attention_outputs3, EMBEDDING_DIM * 2)
...
class myModel0_test(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        with tf.name_scope('target_item_his_users_representation'):
            ...
            item_bh_drink_trm_input = tf.layers.dense(item_bh_drink_trm_input, EMBEDDING_DIM * 2)  # [128,50,38]->[128,50,36]
            # item_bh_drink_trm_input = context_aware_self_attention(item_bh_drink_trm_input, context_weights_matrix, embed_dim=EMBEDDING_DIM * 2, num_units=16, padding_mask=att_mask_input)  # [128,50,36]
            # item_bh_drink_trm_input = context_aware_multi_head_self_attention(item_bh_drink_trm_input, context_weights_matrix, num_heads=4, num_units=8, padding_mask=att_mask_input)
            ...
            # item_bh_drink_trm_input = self_multi_head_attn(item_bh_drink_trm_input, num_units=EMBEDDING_DIM * 2, num_heads=4, padding_mask=att_mask_input, causality_mask_bool=False, dropout_rate=0, is_training=self.is_training, name="multihead_attention_outputs_for_users")
            # # 下面三行是point-wise feed_forward
            ...
            # item_bh_drink_trm_input = item_bh_drink_trm_input1 + item_bh_drink_trm_input
            # self.item_bh_drink_trm_output = transformer_model(item_bh_drink_trm_input, hidden_size=INC_DIM, attention_mask=item_bh_self_att_mask, num_hidden_layers=1, num_attention_heads=2, intermediate_size=256, intermediate_act_fn=gelu, hidden_dropout_prob=0.2, scope='item_bh_drink_trm', attention_probs_dropout_prob=0.2, do_return_all_layers=False)
            self.target_item_his_att_users_output = self_multi_head_attn_v2(item_bh_drink_trm_input, num_units=INC_DIM, num_heads=4, padding_mask=att_mask_input, causality_mask_bool=False, dropout_rate=0.2, is_training=self.is_training, name="target_item_his_att_users_output")
            # ----------------------------------------------------- 处理users的表示 -----------------------------------------------------
            self.context_embedding_for_user = tf.concat([self.position_his_users_eb, self.item_bh_time_embeeded], -1)  # [128,50,2],[128,50,18]->[128,50,20]
...
class myModel1_test(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        with tf.name_scope('target_item_his_users_his_items_representation'):
            ...
            print("target_item_his_user_his_items_avg", target_item_his_user_his_items_avg)
...
class myModel2_test(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        ...
        with tf.name_scope('DMIN_user_module'):
            ...
            with tf.name_scope("multi_head_attention_1"):
                weight_matrix_size = 256  # 18*18=324
                self.his_item_emb_dynamic = tf.layers.dense(tf.concat([self.his_item_emb, self.his_item_user_avg, self.position_his_eb, self.his_items_tiv_emb], axis=-1), INC_DIM)
                his_item_with_tiv_emb = tf.concat([self.position_his_eb, self.his_items_tiv_emb, self.his_item_emb], axis=-1) * tf.expand_dims(self.mask, axis=-1)  # [128,20,2],[128,20,18],[128,20,36]->[128,20,56]
                # 这里到底是sum还是mean，可以试试
                self.his_item_axis1_sum = tf.reduce_sum(his_item_with_tiv_emb, 1)  # [128,56]
                self.his_item_axis2_sum = tf.reduce_sum(his_item_with_tiv_emb, -1)  # [128,20]类似SENET
                ...
                his_item_with_tiv_emb = tf.layers.dense(his_item_with_tiv_emb, EMBEDDING_DIM * 2)
                # multihead_attention_outputs = context_aware_self_attention(his_item_with_tiv_emb, context_weights_matrix, embed_dim=EMBEDDING_DIM * 2, num_units=weight_matrix_size ** 0.5, padding_mask=self.mask)
                multihead_attention_outputs = context_aware_multi_head_self_attention(self.his_item_emb_dynamic, context_weights_matrix, num_heads=4, num_units=8, padding_mask=self.mask)
                # multihead_attention_outputs = self_multi_head_attn(his_item_with_tiv_emb, num_units=EMBEDDING_DIM * 2, num_heads=4, padding_mask=self.mask, causality_mask_bool=False, dropout_rate=0, is_training=self.is_training, name="multihead_attention_outputs_for_items")
                # 下面三行是point-wise feed_forward
                # multihead_attention_outputs1 = tf.layers.dense(multihead_attention_outputs, EMBEDDING_DIM * 4, activation=tf.nn.relu)
                # multihead_attention_outputs1 = tf.layers.dense(multihead_attention_outputs1, EMBEDDING_DIM * 2)
        ...
        # inp = tf.concat([inp, self.item_user_his_eb_sum, item_user_his_eb_att_sum, interest_emb], 1)  # [128,260],[128,36],[128,36],[128,36]->[128,368]
        # interest_emb,self.his_item_user_att_sum, self.his_item_user_att_sum * item_user_his_eb_att_sum, self.his_item_user_att_sum * self.item_user_his_eb_sum,
        inp = tf.concat([inp, interest_emb, current_user_interest_emb * self.target_item_emb, self.item_user_his_eb_sum, self.item_user_his_eb_sum * self.user_embedded], 1)  # [128,260],[128,36],[128,36],[128,36]->[128,368]
        self.build_fcn_net(inp, use_dice=True)
...
class Model_MIN(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False):
        ...
        # 截断目标物品的购买用户的长度和时间长度：mapping_to_k处理一个给定的序列（或批量序列），使得每个序列中只保留最后 k 个元素，并生成一个对应的掩码。item_bh_seq_len_t返回的是截断后的真实长度
        # 此处的目的是为了方便修改利用的用户序列长度，而不用每次都在prepare_data中修改，prepare_data中保留较长的序列，这里可以随意控制序列的长短
        self.item_bh_k_user_emb, self.item_bh_seq_len_t, self.item_bh_mask_t = mapping_to_k(self.item_user_his_uid_batch_embedded, self.seq_len_u_ph, k=SEQ_USER_T)  # [128,50,18] 得到[128,k,18],[128],[128,k,1]
        self.item_bh_k_user_items_emb, _, _ = mapping_to_k(tf.reshape(self.item_user_his_items_emb, tf.shape(self.item_user_his_items_emb)[0], tf.shape(self.item_user_his_items_emb)[1], -1), self.seq_len_u_ph, k=SEQ_USER_T)  # [128,50,20,18]->[128,50,20*18] 得到[128,k,20*18],[128],[128,k,1]
